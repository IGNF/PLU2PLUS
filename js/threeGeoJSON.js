...
function drawThreeGeo(json, radius, shape, lineMat, mat, couche) {
    ...
    for (var geom_num = 0; geom_num < json_geom.length; geom_num++) {
        ...
        if (json_geom[geom_num].type == 'Point') {
            ...
            //var data = earcut.flatten(json_geom[geom_num].coordinates);                      
            for (var segment_num = 0; segment_num < json_geom[geom_num].coordinates.length; segment_num++) {
                ...
                drawLine(y_values, z_values, x_values, lineMat, couche);*/
                for (var point_num = 0; point_num < coordinate_array.length; point_num++) {
                        x_values.push(coordinate_array[point_num][0]); 
                        y_values.push(coordinate_array[point_num][1]); 
                        z_values.push(coordinate_array[point_num][2]); 
                    }
                    //var data = earcut.flatten(coordinate_array);
                    drawLine(x_values, y_values, z_values, lineMat, mat, couche);
            }                            
...
function drawLine(x_values, y_values, z_values, lineMat, mat, couche) {
    var line_geom = new THREE.Geometry();
    var mesh_geom = new THREE.Geometry();
    createVertexForEachPoint(line_geom, x_values, y_values, z_values);
    createVertexForEachPoint(mesh_geom, x_values, y_values, z_values);
    //mesh_geom.vertices.push(new THREE.Vector3(x_values[0], y_values[0], z_values[0]));

    /*for( var i = 0; i < mesh_geom.vertices.length - 2; i=i+3 ){

        mesh_geom.faces.push( new THREE.Face3( mesh_geom.vertices[i], mesh_geom.vertices[i+1], mesh_geom.vertices[i+2] ));

    }*/

    //mesh_geom.faces.push( new THREE.Face3( mesh_geom.vertices[4], mesh_geom.vertices[3], mesh_geom.vertices[2] ));
    //mesh_geom.faces.push( new THREE.Face3( mesh_geom.vertices[2], mesh_geom.vertices[1], mesh_geom.vertices[4] ));
    ...
    for( var i = 0; i < mesh_geom.vertices.length-1; i++ ){

        offset = (i % 2 === 0) ? 0.001+i/1000 : -0.001-i/1000;
        mesh_geom.vertices[i].x += offset;

    }*/


    //mesh_geom.applyMatrix( new THREE.Matrix4().makeRotation( 1, 0, 0 ) );
    //mesh_geom.rotateX(1);
    //mesh_geom.verticesNeedUpdate = true;

    //earcut
    //mesh_geom.vertices[mesh_geom.vertices.length-1] = decal;

    /*THREE.Triangulation.setLibrary('earcut');
        var coords = [];
    for( var i = 0; i < mesh_geom.vertices.length; i++ ){

        coords.push(mesh_geom.vertices.x);
        coords.push(mesh_geom.vertices.y);
        coords.push(mesh_geom.vertices.z);

    }
    triangles = earcut(coords, null, 3)*/


    triangles = THREE.ShapeUtils.triangulateShape ( mesh_geom.vertices, holes );

    for( var i = 0; i < triangles.length; i++ ){

        /*if (triangles[i][0] % (mesh_geom.vertices.length-1) === triangles[i][1] % (mesh_geom.vertices.length-1) ){
            triangles[i][1] -= 2;
        }*/

        mesh_geom.faces.push( new THREE.Face3( triangles[i][0], triangles[i][1], triangles[i][2] ));

    }


                
    var line_material = lineMat;//new THREE.LineBasicMaterial(options);
    var line = new THREE.Line(line_geom, line_material);
    var mesh = new THREE.Mesh(mesh_geom, mat)

    //moveMesh(line,couche.position.displacement.x, couche.position.displacement.y,couche.position.displacement.z);
    moveMesh(line,-1051199.90836475,-6840580.61289425, -140 );
    moveMesh(mesh,-1051199.90836475,-6840580.61289425, -140 );
